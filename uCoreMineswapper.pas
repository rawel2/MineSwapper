...
type
  ...
  private
    ...
    mFieldWidth:  Word;
    mMineCount: Word;
    mFieldUp: array of array of TCellType;
    mFieldDown: array of array of TCellType;
...
begin
  for X := Low(mFieldUp) to High(mFieldUp) do
    for Y := Low(mFieldUp[0]) to High(mFieldUp[0]) do
      if IsMine(X,Y) then
...
begin
  ...
  for X := Low(mFieldDown) to High(mFieldDown) do
    for Y := Low(mFieldDown[0]) to High(mFieldDown[0]) do
      mFieldDown[X,Y] := c0;
